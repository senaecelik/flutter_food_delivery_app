import 'package:flutter/material.dart';
import 'package:google_fonts/google_fonts.dart';

class TextStyleConstant {
  static TextStyle text =
      GoogleFonts.varela(fontSize: 17, fontWeight: FontWeight.w600);
  static TextStyle bigText =
      GoogleFonts.varela(fontSize: 28, fontWeight: FontWeight.w600);
  static TextStyle largeText =
      GoogleFonts.varela(fontSize: 22, fontWeight: FontWeight.w600);
  static TextStyle title1 =
      GoogleFonts.varela(fontSize: 34, fontWeight: FontWeight.w400);
  static TextStyle text2 =
      GoogleFonts.varela(fontSize: 15, fontWeight: FontWeight.w400);
}
